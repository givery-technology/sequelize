...
module.exports = (function() {
  ...
  Object.defineProperty(Instance.prototype, 'isDeleted', {
    get: function() {
      return this.Model._timestampAttributes.deletedAt && this.dataValues[this.Model._timestampAttributes.deletedAt] !== null && this.dataValues[this.Model._timestampAttributes.deletedAt] !== 0;
    }
  });
        ...
        if (this.options && this.options.include && this.options.includeNames.indexOf(key) !== -1 && value) {
          ...
          if (!options.raw) {
            ...
            if (this.Model._hasDateAttributes && this.Model._isDateAttribute(key) && value !== null && value !== undefined && !(value instanceof Date) && !value._isSequelizeMethod) {
              // Fix raw string input that would work with SQL but not Sequelize
              if (typeof value === 'string') {
                // Replace month by '01' if it was '00'
                // e.g. xxxx-00-xx -> xxxx-01-xx
                if (value.length >= 7 && value.charAt(5) === '0' && value.charAt(6) === '0') {
                  value = value.substring(0, 5) + '01' + value.substring(7, value.length);
                }
                // Replace date by '01' if it was '00'
                // e.g. xxxx-xx-00 -> xxxx-xx-01
                if (value.length >= 10 && value.charAt(8) === '0' && value.charAt(9) === '0') {
                  value = value.substring(0, 8) + '01' + value.substring(10, value.length);
                }
              }
  ...
  */
  ...
  Instance.prototype.destroy = function(options) {
    ...
    return Promise.bind(this).then(function() {
      ...
      if (this.Model._timestampAttributes.deletedAt && options.force === false) {
        this.setDataValue(this.Model._timestampAttributes.deletedAt, Math.floor(Date.now() / 1000));
  ...
  Instance.prototype.restore = function(options) {
    ...
    return Promise.bind(this).then(function() {
      ...
      this.setDataValue(this.Model._timestampAttributes.deletedAt, 0);
